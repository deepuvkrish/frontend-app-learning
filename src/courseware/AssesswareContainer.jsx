  ...
  if (courseStatus === 'loaded' && !sequenceId) {
    ...
    getResumeBlock(courseId).then((data) => {
      ...
      if (data.sectionId && data.unitId) {
        history.replace(`/assessment/${courseId}/${data.sectionId}/${data.unitId}`);
      } else if (firstSequenceId) {
        history.replace(`/assessment/${courseId}/${firstSequenceId}`);
      }
    });
  }
});
...
const checkSectionUnitToUnitRedirect = memoize((courseStatus, courseId, sequenceStatus, section, unitId) => {
  if (courseStatus === 'loaded' && sequenceStatus === 'failed' && section && unitId) {
    history.replace(`/assessment/${courseId}/${unitId}`);
  }
});
...
const checkSectionToSequenceRedirect = memoize((courseStatus, courseId, sequenceStatus, section, unitId) => {
  if (courseStatus === 'loaded' && sequenceStatus === 'failed' && section && !unitId) {
    ...
    if (section.sequenceIds && section.sequenceIds[0]) {
      history.replace(`/assessment/${courseId}/${section.sequenceIds[0]}`);
    // Otherwise, just go to the course root, letting the resume redirect take care of things.
    } else {
      history.replace(`/assessment/${courseId}`);
    }
  }
});
    ...
    if (courseStatus === 'loaded' && sequenceStatus === 'failed' && !section && !routeUnitId) {
      if (sequenceMightBeUnit) {
        ...
        getSequenceForUnitDeprecated(courseId, unitId).then(
          parentId => {
            if (parentId) {
              history.replace(`/assessment/${courseId}/${parentId}/${unitId}`);
            } else {
              history.replace(`/assessment/${courseId}`);
            }
          },
        ...
        // Invalid sequence that isn't a unit either. Redirect up to main assessment.
        history.replace(`/assessment/${courseId}`);
      }
    }
  },
...
const checkSequenceToSequenceUnitRedirect = memoize((courseId, sequenceStatus, sequence, unitId) => {
  if (sequenceStatus === 'loaded' && sequence.id && !unitId) {
    if (sequence.unitIds !== undefined && sequence.unitIds.length > 0) {
      ...
      // This is a replace because we don't want this change saved in the browser's history.
      history.replace(`/assessment/${courseId}/${sequence.id}/${nextUnitId}`);
    }
  }
});
...
const checkSequenceUnitMarkerToSequenceUnitRedirect = memoize((courseId, sequenceStatus, sequence, unitId) => {
  ...
  if (unitId === 'first') {
    if (hasUnits) {
      ...
      history.replace(`/assessment/${courseId}/${sequence.id}/${firstUnitId}`);
    } else {
      // No units... go to general sequence page
      history.replace(`/assessment/${courseId}/${sequence.id}`);
    }
  } else if (unitId === 'last') {
    if (hasUnits) {
      const lastUnitId = sequence.unitIds[sequence.unitIds.length - 1];
      history.replace(`/assessment/${courseId}/${sequence.id}/${lastUnitId}`);
    } else {
      // No units... go to general sequence page
      history.replace(`/assessment/${courseId}/${sequence.id}`);
    }
...
class AssesswareContainer extends Component {
  ...
  handleUnitNavigationClick = (nextUnitId) => {
    ...
    history.push(`/assessment/${courseId}/${sequenceId}/${nextUnitId}`);
  };
  ...
  handleNextSequenceClick = () => {
    ...
    if (nextSequence !== null) {
      history.push(`/assessment/${courseId}/${nextSequence.id}/first`);
  ...
  handlePreviousSequenceClick = () => {
    ...
    if (previousSequence !== null) {
      history.push(`/assessment/${courseId}/${previousSequence.id}/last`);
    }
  };
...
AssesswareContainer.defaultProps = {
  courseId: null,
  sequenceId: null,
